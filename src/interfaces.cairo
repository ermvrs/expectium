...
use expectium::types::{
    Asset, PlatformFees, MarketData, OrdersData, UserData, SharesState, SharesUserInfoResponse,
    SharesInfoResponse
};
...
trait IOrderbook<TContractState> {
    fn insert_buy_order(
        ref self: TContractState, asset: Asset, quote_amount: u256, price: u16
    ) -> u32; // order_id döndürür
    ...
    // views
    fn get_order(
        self: @TContractState, asset: Asset, side: u8, order_id: u32
    ) -> felt252; // packed order döndürür. TODO: direk order döndürülebilir.
...
trait IFactory<TContractState> {
    // externals
    fn create_market(
        ref self: TContractState, resolver: ContractAddress, collateral: ContractAddress
    ) -> (u64, ContractAddress); // market id döndürür.
    // views
...
trait IDistributor<TContractState> {
    ...
    ); // Distributiona para ekler.
    fn claim(
        ref self: TContractState, token: ContractAddress, share_id: u256
    ); // tokenid ye ait token fee yi claim eder.
    // views
...
trait IStatistics<TContractState> {
    ...
    fn get_user_market_trades_count(
        self: @TContractState, user: ContractAddress, orderbook: ContractAddress
    ) -> u64;
    fn get_user_market_volume(
        self: @TContractState, user: ContractAddress, orderbook: ContractAddress
    ) -> u256;
    // operator
    fn register_market(
        ref self: TContractState, orderbook: ContractAddress, market: ContractAddress
    );
    fn transfer_operator(ref self: TContractState, new_operator: ContractAddress);
    fn upgrade_contract(ref self: TContractState, new_class: ClassHash);
}
...
trait IMulticall<TState> {
    ...
    ) -> SharesUserInfoResponse; // u256 gerek yok
    fn get_shares_info(
        self: @TState,
        shares: ContractAddress,
        distributor: ContractAddress,
    ...
    fn aggregateUserData(
        self: @TState, user: ContractAddress, market: ContractAddress, orderbook: ContractAddress
    ) -> UserData;
    fn aggregateMarketData(
        self: @TState, market: ContractAddress, orderbook: ContractAddress
    ) -> MarketData;
