...
fn setup_with_mergeshares() -> Config {
    ...
    operator
        .erc20_transfer(collateral.contract_address, cindy.contract_address, 2000000000000000000);
    ...
    let orderbook = deploy::deploy_orderbook(
        market, operator.contract_address, collateral.contract_address, distributor.contract_address
    );
    ...
    cindy
        .erc20_approve(
            collateral.contract_address, market, integer::BoundedInt::max()
        ); // 0.2 ether
...
fn test_insert_buy_order_without_approval() {
    ...
    alice
        .orderbook_insert_buy_order(
            book.contract_address, Asset::Happens(()), 100000_u256, 1000_u16
        );
}
...
fn test_insert_buy_order_exceeds_balance() {
    ...
    alice
        .orderbook_insert_buy_order(book.contract_address, Asset::Happens(()), balance * 5, 10_u16);
}
...
fn test_insert_buy_order() {
    ...
    let order_id = alice
        .orderbook_insert_buy_order(
            book.contract_address, Asset::Happens(()), 1000000000000000, 200_u16
        );
...
fn test_insert_buy_orders_check_sorting() {
    ...
    let order_id = alice
        .orderbook_insert_buy_order(
            book.contract_address, Asset::Happens(()), 1000000000000000, 100_u16
        );
    ...
    let order_id = bob
        .orderbook_insert_buy_order(
            book.contract_address, Asset::Happens(()), 100000000000000, 150_u16
        );
    assert(order_id == 2, 'order_id wrong');

    let order_id = alice
        .orderbook_insert_buy_order(
            book.contract_address, Asset::Happens(()), 70000000000000, 125_u16
        );
...
fn test_insert_sell_order_check_match() {
    ...
    let order_id = alice
        .orderbook_insert_buy_order(book.contract_address, Asset::Happens(()), 100000000, 100_u16);
    ...
    let before_trade_alice_commodity_balance = market
        .balance_of(alice.contract_address, Asset::Happens(()));
    let before_trade_bob_collateral_balance = collateral.balanceOf(bob.contract_address);

    let order_id = bob
        .orderbook_insert_sell_order(
            book.contract_address, Asset::Happens(()), alice_order_amount * 2, 75_u16
        );
    assert(order_id == 2, '2nd order_id wrong');

    let after_trade_alice_commodity_balance = market
        .balance_of(alice.contract_address, Asset::Happens(()));
    let after_trade_bob_collateral_balance = collateral.balanceOf(bob.contract_address);

    let commodity_bought =
        alice_order_amount; // 10aaa usdc ödedim, miktar bp = 10aaa * 10000, tanesine 100 ödedim 10aaa* 100 olması lazım
    // Alice commodity balance arttı çünkü alım emri girmişti
    assert(
        (after_trade_alice_commodity_balance
            - before_trade_alice_commodity_balance) == commodity_bought,
        'commodity transfer wrong'
    );
    ...
    assert(
        (after_trade_bob_collateral_balance
            - before_trade_bob_collateral_balance) == collateral_taken,
        'collateral transfer wrong'
    );
    ...
    assert(
        order.amount == ((alice_order_amount * 2) - commodity_bought).try_into().unwrap(),
        'amount wrong'
    );
...
fn test_insert_sell_order_and_cancel() {
    ...
    let order_id = bob
        .orderbook_insert_sell_order(
            book.contract_address, Asset::Happens(()), 15000000000000, 75_u16
        );
    assert(order_id == 1, 'order_id wrong');
...
fn test_insert_sell_orders_check_sorting() {
    ...
    let order_id = alice
        .orderbook_insert_sell_order(
            book.contract_address, Asset::Happens(()), 4000000000000, 75_u16
        );
    ...
    let order_id = alice
        .orderbook_insert_sell_order(
            book.contract_address, Asset::Happens(()), 15000000000000, 125_u16
        );
...
fn test_trade_with_fees() {
    ...
    let first_order_id = alice
        .orderbook_insert_buy_order(
            book.contract_address, Asset::Happens(()), 100000000, 1000
        ); // 100000000 usdc paid
    ...
    let second_order_id = bob
        .orderbook_insert_sell_order(book.contract_address, Asset::Happens(()), 20000000000, 1000);
    ...
    assert(
        (bob_collateral_balance_after
            - bob_initial_collateral_balance) == (sale_quote_amount - sale_taker_fee),
        'bob collateral wrong'
    );
    ...
    assert(
        (alice_share_balance_after
            - alice_initial_share_balance) == (alice_buy_amount - sale_shares_fee),
        'alice shares wrong'
    );
    ...
    assert(
        unpacked_second_order.amount == (20000000000 - alice_buy_amount).try_into().unwrap(),
        'order amount left wrong'
    );
...
fn test_taker_match_with_multiple_orders() {
    ...
    alice
        .erc20_approve(
            collateral.contract_address, book.contract_address, integer::BoundedInt::max()
        );
    bob
        .erc20_approve(
            collateral.contract_address, book.contract_address, integer::BoundedInt::max()
    ...
    alice
        .orderbook_insert_buy_order(
            book.contract_address, Asset::Not(()), 10000000, 2000
        ); // buy with 10000000 usdc
    bob
        .orderbook_insert_buy_order(
            book.contract_address, Asset::Not(()), 50000000, 1750
        ); // buy with 50000000 usdc
    alice
        .orderbook_insert_buy_order(
            book.contract_address, Asset::Not(()), 20000000, 1500
        ); // buy with 20000000 usdc

    // total 80000000 on sale

    cindy.market_approve(market.contract_address, book.contract_address);

    let cindys_order_id = cindy
        .orderbook_insert_sell_order(
            book.contract_address, Asset::Not(()), 1000000000, 1000
        ); // sells 100000000 with price 0.1$
    ...
    assert(
        cindys_order
            .amount == (1000000000
                - (alice_first_order_amount + bob_first_order_amount + alice_second_order_amount)),
        'cindys order amount wrong'
    );
    ...
    let cindy_return_without_fee: u256 = (((alice_first_order_amount * 2000)
        + (bob_first_order_amount * 1750)
        + (alice_second_order_amount * 1500))
        .into())
        / 10000;
    ...
    assert(
        (cindy_final_balance
            - cindy_initial_collateral_balance) == (cindy_return_without_fee - cindy_taker_fee),
        'cindy coll balance wrong'
    );
    ...
    let alice_maker_fee: u256 = (((alice_first_order_amount + alice_second_order_amount) * 300)
        / 10000)
        .into();
    let bob_maker_fee: u256 = ((bob_first_order_amount * 300) / 10000).into();

    assert(
        (alice_after_share_balance
            - alice_initial_share_balance) == ((alice_first_order_amount
                + alice_second_order_amount)
                - alice_maker_fee.try_into().unwrap())
            .into(),
        'alice fee wrong'
    );
    assert(
        (bob_after_share_balance
            - bob_initial_share_balance) == (bob_first_order_amount.into() - bob_maker_fee),
        'bob fee wrong'
    );
...
fn test_taker_match_with_multiple_orders_but_final_remains() { // TODO
    ...
    alice
        .orderbook_insert_buy_order(
            book.contract_address, Asset::Not(()), alice_first_order_quote, 2000
        ); // buy with 10000000 usdc 
    bob
        .orderbook_insert_buy_order(
            book.contract_address, Asset::Not(()), bob_first_order_quote, 1750
        ); // buy with 50000000 usdc
    let final_order_id = alice
        .orderbook_insert_buy_order(
            book.contract_address, Asset::Not(()), alice_second_order_quote, 1500
        ); // buy with 20000000 usdc
    ...
    let cindys_order_id = cindy
        .orderbook_insert_sell_order(
            book.contract_address, Asset::Not(()), 70000000, 1000
        ); // sells 70000000 with price 0.1$
    ...
    assert(
        final_order
            .amount == (alice_second_order_amount - left_for_final_order)
            .try_into()
            .unwrap(),
        'final order amount wrong'
    );

    let cindy_return_without_fee: u256 = (((alice_first_order_amount * 2000)
        + (bob_first_order_amount * 1750)
        + (left_for_final_order * 1500))
        .into())
        / 10000;
    let cindy_taker_fee = (cindy_return_without_fee * 500) / 10000;
    ...
    let alice_maker_fee: u256 = (((alice_first_order_amount + (left_for_final_order)) * 300)
        / 10000)
        .into();
    let bob_maker_fee: u256 = ((bob_first_order_amount * 300) / 10000).into();
    ...
    assert(
        (alice_after_share_balance
            - alice_initial_share_balance) == ((alice_first_order_amount + (left_for_final_order))
                - alice_maker_fee)
            .into(),
        'alice fee wrong'
    );
    assert(
        (bob_after_share_balance
            - bob_initial_share_balance) == (bob_first_order_amount - bob_maker_fee)
            .into(),
        'bob fee wrong'
    );
...
fn test_taker_match_with_multiple_orders_but_maker_is_seller_and_final_remains() {
    ...
    let alice_first_order_id = alice
        .orderbook_insert_sell_order(
            book.contract_address, Asset::Happens(()), alice_first_order_amount, 2250
        );
    assert(alice_first_order_id == 1, 'alice first orderid wrong');

    // bob sells 4000000 per 0.07$

    let bob_first_order_id = bob
        .orderbook_insert_sell_order(
            book.contract_address, Asset::Happens(()), bob_first_order_amount, 700
        );
    assert(bob_first_order_id == 2, 'bob orderid wrong');

    let alice_second_order_id = alice
        .orderbook_insert_sell_order(
            book.contract_address, Asset::Happens(()), alice_second_order_amount, 1000
        );
    ...
    let cindy_spend_quote = (((alice_first_order_amount - 1000000) * 2250) / 10000)
        + ((bob_first_order_amount * 700) / 10000)
        + ((alice_second_order_amount * 1000) / 10000);

    let cindy_order_id = cindy
        .orderbook_insert_buy_order(
            book.contract_address, Asset::Happens(()), cindy_spend_quote, 2500
        );
    assert(
        cindy_order_id == 0, 'cindy order entered'
    ); // orderid should return 0 bcs it already matches all amount
    ...
    assert(
        (cindy_final_share_balance) == (cindy_initial_share_balance
            + (cindy_bought_amount - cindy_taker_fee)),
        'cindy final share wrong'
    );

    let alice_collateral_receive_without_fee: u256 = (((alice_first_order_amount - 1000000) * 2250)
        / 10000)
        + ((alice_second_order_amount * 1000) / 10000);
    ...
    assert(
        (alice_final_collateral_balance
            - alice_initial_collateral_balance) == (alice_collateral_receive_without_fee
                - alice_maker_fee),
        'alice coll wrong'
    );
    assert(
        (bob_final_collateral_balance
            - bob_initial_collateral_balance) == (bob_collateral_receive_without_fee
                - bob_maker_fee),
        'bob coll wrong'
    );
...
fn test_taker_match_with_multiple_orders_and_taker_is_seller() {
    ...
    alice
        .orderbook_insert_sell_order(
            book.contract_address, Asset::Not(()), alice_first_order_amount, 1000
        ); // sell for 0.1$

    bob
        .orderbook_insert_sell_order(
            book.contract_address, Asset::Not(()), bob_first_order_amount, 450
        );

    alice
        .orderbook_insert_sell_order(
            book.contract_address, Asset::Not(()), alice_second_order_amount, 1250
        );

    let total_amount_on_sale = alice_first_order_amount
        + bob_first_order_amount
        + alice_second_order_amount;
    ...
    let cindys_order_id = cindy
        .orderbook_insert_buy_order(book.contract_address, Asset::Not(()), cindy_total_quote, 2000);
    ...
    assert(
        cindys_order.amount == (cindy_quote_left * 10000 / 2000).try_into().unwrap(),
        'cindys order amount wrong'
    );
    ...
    assert(
        (cindy_final_share_balance
            - cindy_initial_share_balance) == (cindy_return_without_fee - cindy_taker_fee),
        'cindy share balance wrong'
    );

    assert(
        (cindy_initial_collateral_balance - cindy_final_collateral_balance) == cindy_total_quote,
        'cindy coll balance wrong'
    );
    ...
    let alice_maker_fee: u256 = (((((10000000 * 1000) / 10000) + ((40000000 * 1250) / 10000))
        * 300_u256)
        / 10000)
        .into();
    ...
    assert(
        (alice_after_collateral_balance
            - alice_initial_collateral_balance) == (alice_return_without_fee - alice_maker_fee),
        'alice coll wrong'
    );
    assert(
        (bob_after_collateral_balance
            - bob_initial_collateral_balance) == (bob_return_without_fee - bob_maker_fee),
        'bob coll wrong'
    );
    // check distribution.
