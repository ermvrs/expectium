...
#[test]
#[available_gas(1000000000)]
fn mint_shares_and_multicall_different_contracts_with_failing_call() {
    let setup = setup();
    let market = setup.market;
    let multicall = setup.multicall;
    let factory = setup.factory;
    ...
    let call_s: Call = Call {
        contract: factory.contract_address,
        entrypoint : 0x2a52cf33029de9f84853745a87af28464c80bf0346df1b32e205fc73319f633, // unknown entrypoint
        calldata: array![]
    };
    ...
    loop {
        ...
        if(i == 0) {
            let result_first = *result.at(0);
            assert(*multicall_result.at(i).status == true, 'status wrong');
            assert(result_first.into() == 100000000000000000, 'total supply wrong');
        }

        if(i == 1) {
            assert(*multicall_result.at(i).status == false, 'status wrong');
        }
