...
use expectium::constants::{
    SHIFT_32, SHIFT_96, SHIFT_128, SHIFT_192, SHIFT_208, SHIFT_216, SHIFT_224, SHIFT_240,
    UNSHIFT_32, UNSHIFT_64, UNSHIFT_128, UNSHIFT_16, UNSHIFT_8
};
use expectium::implementations::{
    OrderStatusIntoU256, OrderStatusIntoFelt252, Felt252TryIntoOrderStatus, AssetIntoFelt252
};
...
fn pack_order(order: Order) -> felt252 { // TEST EDİLDİ DOĞRU GİBİ DURUYOR.
    ...
    shifted = shifted
        | (u256_from_felt252(order.status.into())
            * SHIFT_240); // KONTROL EDİLMELİ PACK DÜZGÜN MÜ.

    shifted.try_into().unwrap()
}

fn unpack_order(
    packed_order: felt252
) -> Order { // TEST EDİLDİ DOĞRU GİBİ DURUYOR. EN YÜKSEK DEĞERLERLE TEST EDİLMELİ.
    let unshifted: u256 = packed_order.into();

    let order_id: u32 = (unshifted & UNSHIFT_32).try_into().unwrap();
    let date: u64 = ((unshifted / SHIFT_32) & UNSHIFT_64)
        .try_into()
        .unwrap(); // burada libfuncs problemi var
